...
$(document).ready(function ()
{
	...
	$("#canned_query_list").change(function(e)
	{
  		//var my_canned_query_id = $(this).children(":selected", "selectedOptions").attr("id");
		var my_canned_query_id = $("#canned_query_list :selected").val();
		if(my_canned_query_id != -1)
		{
...
function makeCannedQuerySelection()
{
	...
	postData(strUrl, data, function(json)
	{
		...
		html += '<optgroup ><option class="service-small" value="-1">Choose Operation</option>';
		for(var i=0; i < json.fields.length; i++)
		{
			var field = json.fields[i];
			html += '<option class="service-small" value="' + field.canned_query_id + '">' + field.name + '</option>';
