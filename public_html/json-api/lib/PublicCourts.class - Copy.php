...
class MtcCourtReservationHelper extends MtcCourtReservation
{
	...
	public function quick_res($postArray)
	{
		...
		if($this->loadParams($postArray, $params))
		{
			...
			if(isset($postArray['status']))
			{
				$params['status'] = $postArray['status'];
				$sql = 'SELECT date, court_reservation_id, start_time , end_time, member1_id, status, court_id FROM `' . $this->table_name . '` WHERE date = :date AND start_time >= :start_time AND start_time < :end_time AND status = :status ORDER BY start_time ASC';
			}
			else
			{
				$sql = 'SELECT date, court_reservation_id, start_time , end_time, member1_id, status, court_id FROM `' . $this->table_name . '` WHERE date = :date AND start_time >= :start_time AND start_time < :end_time ORDER BY start_time ASC';
			}
			*/
